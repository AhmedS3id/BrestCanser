...
public class NotificationService : INotificationService
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		var notification = new Notification
		{
			...
			Message = $"Your scan result: {history.Diagnosis} ({history.Status}) with {(history.Confidence * 100):F1}% confidence."
		};
	...
	new Dictionary<string, string>
		{
				...
				{ "{{Confidence}}", (history.Confidence * 100).ToString("F0") },
				{ "{{Year}}",       DateTime.UtcNow.Year.ToString() }
		});


		await _emailSender.SendEmailAsync(user.Email!, "Your Breast Cancer Scan Result", htmlBody);
